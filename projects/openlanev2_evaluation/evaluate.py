                    ...
                    for i in range(preds_topology_lcte.shape[0]):
                        for j in range(preds_topology_lcte.shape[1]):
                            if i in gt_pred_lane_centerline and j in gt_pred_traffic_element:
                                preds_topology_lcte[i][j] = preds_topology_lcte_unmatched[
                                    gt_pred_lane_centerline[i]][gt_pred_traffic_element[j]]
...
def evaluate(ground_truth, predictions, verbose=True, vertical_lower=-50.0, vertical_upper=50.0):
    r"""
    ...
    f1 = LaneEval(vertical_lower=vertical_lower, vertical_upper=vertical_upper)
